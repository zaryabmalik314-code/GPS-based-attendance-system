...
- Trusting a client-supplied embedding also means the backend cannot verify
...
def get_best_face_embedding(image_b64_list: List[str]) -> dict:
    ...
    if best_face is None or best_score < 0:
        return {"embedding": None, "quality": 0.0, "reason": "no_usable_face_detected"}

    if float(getattr(best_face, "det_score", 0.0)) < MIN_DETECTION_CONFIDENCE:
        return {"embedding": None, "quality": best_score, "reason": "low_detection_confidence"}

    embedding = best_face.normed_embedding  # InsightFace already L2-normalizes this
    return {"embedding": embedding.tolist(), "quality": best_score, "reason": None}
...
def str_to_embeddings(s: str) -> List[np.ndarray]:
    ...
    enrolled faculty do not break - they will just have 1 embedding until
    they re-enroll.
    """
    s = s.strip()
    if s.startswith("["):
...
def enroll_from_frames(image_b64_list: List[str]) -> dict:
    ...
    Returns {"embeddings": [[...], [...], [...]], "rejected": [indices]}
    """
    embeddings = []
    rejected = []
    ...
    if not embeddings:
        return {"embeddings": [], "rejected": rejected, "reason": "no_usable_face_in_any_frame"}

    return {"embeddings": embeddings, "rejected": rejected, "reason": None}
